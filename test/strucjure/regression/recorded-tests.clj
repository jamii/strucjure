...
(used (->Output (->Name (quote a) 1) (fnk [a] (clojure.core/+ a 1))))
((eval (pattern->view (->Output (->Name (quote a) 1) (fnk [a] (clojure.core/+ a 1))) true true)) 1)
((eval (pattern->view (->Output (->Name (quote a) 1) (fnk [a] (clojure.core/+ a 1))) true true)) 2)
...
(pattern->view (->Output (zero-or-more 1) (fnk [] (quote ones))) true true)
(pattern->view (->Output (->Name (quote a) (zero-or-more 1)) (fnk [a] a)) true true)
...
((eval (pattern->view (list (->Rest (->Name (quote elems) (zero-or-more (->Any))))) true true)) (list 1 2 3))
((eval (pattern->view (->Seqable [(->Rest (zero-or-more [(->Any) (->Any)]))]) true true)) (quote {:foo 1, :bar (& * 3)}))
((eval (pattern->view (->And [{} (->Name (quote elems) (->Seqable [(->Rest (zero-or-more [(->Any) (->Any)]))]))]) true true)) (quote {:foo 1, :bar (& * 3)}))
...
(pattern->view (->Is (fn* [p1__22142#] (symbol? p1__22142#))) true true)
...
(macroexpand-1 (quote (view (clojure.core/unquote (->NodeOf num-out (quote num))))))
(graph->view (quote num) num-out)
((view (clojure.core/unquote (->NodeOf num-out (quote num)))) (quote (succ (succ zero))))
(macroexpand (quote (match 1 1 :ok)))
(match 1 1 :ok)
(match 1 2 :not-ok)
...
(def x [1 2])
(match [1 2] (clojure.core/unquote x) :ok)
(match [1 2 3] (clojure.core/unquote x) :not-ok)
(match [1 2 3] [1 2 ^x _] x)
(match [1 2 3] [1 2 ^x & _] x)
(pattern [1 2 (clojure.core/unquote (? 3))])
(view [1 2 (clojure.core/unquote (? 3))])
(match [1 2 3] [1 2 & (clojure.core/unquote (? ^x _))] x)
(match [1 2] [1 2 & (clojure.core/unquote (? ^x _))] x)
((view [& * 1 & ? 2 3]) [1 1 1 2 3])
((view [& * 1 & ? 2 3]) [1 1 1 3])
((view [& * 1 & ? 2 3]) [1 1 1 2 2 3])
((view [& * 1 & ? 2 3]) [1 1 1])