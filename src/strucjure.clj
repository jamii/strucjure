...
(def empty-view
  (->View nil (fn [input true-case false-case] (false-case))))
...
(def clj->clj* empty-view)

(defn clj->clj [clj]
  clj
  ;; (prewalk-expand clj->clj* (clojure.walk/macroexpand-all clj))
  )

;; Views can be registered to modify the low-level AST produced by the compiler

(def last->last* empty-view)

(defn last->last [last]
  (prewalk-expand last->last* last))
...
(defn case->hast [patterns&values]
  ...
  (->Or
   (for [[pattern value] (partition 2 patterns&values)]
     (seq-ast (pattern->hast pattern) (->Return value)))))
